    ...
    return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        ...
        if err == nil {
            ...
            if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
                ...
                user, err = repository.L("user").(*repository.UserRepository).FindByID(userId)
                if err != nil {
                    log.Println(err)
                }
                if _, ok = claims["openid"]; ok {
                    user.Openid = claims["openid"].(string)
                }
            } else {
                log.Println(err)
            }
...
func validateBearerAuthHeader(ctx context.Context, r *http.Request) (*jwt.Token, error) {
    ...
    token, err := ctx.Value("authService").(*service.AuthService).ValidateJWT(ctx, &tokenString)
    return token, err
}
